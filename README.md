...
The rules and guidelines generated by this tool are designed using professional Agentic Alignment methodologies:
- **`andrej-karpathy-skills`**: Enforces "Simplicity First", "Surgical Changes", and "Think Before Coding" logic.
- **`superpowers-skills`**: Implements "Verification Before Completion", "Subagent-Driven Development", "TDD for Documentation", and "Deep Context Savings".
...
> 🌟 **What's New in v1.9.3 (2026-05-02):** Implemented **Deep Context Savings**, **Subagent Orchestration**, and advanced Agentic Alignment based on the `superpowers` and `andrej-karpathy-skills` methodologies. The generator now enforces strict Test-Driven Documentation, "Verification Before Completion", and prevents context bloat by generating pointers instead of dumping large code snippets into `.cursorrules`. We practice what we preach: this repository itself is actively developed and maintained using these exact skills!
